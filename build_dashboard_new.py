...
# Optional: real x/y positions for shovels & dump locations (Cycle Map). CSV columns: Location, X, Y
# (X/Y in any consistent linear units — metres of mine grid easting/northing recommended). Keys must
# match the location ids used in loads (e.g. S013, CR2_MRM, DU_LWR_CADILLAC_518). Missing file → the
# map falls back to a distance-derived layout.
def _cf(v):
...
def _loadside_csv():
    for fname in ('ShovelLoadingSide.csv','ShovelLoadingSideTagLog.csv'):
        try:
            rows=load_csv(fname)
            # normalise Dtl_-prefixed column names produced by the TagLog export
            out=[]
            for r in rows:
                n={k[4:] if k.startswith('Dtl_') else k: v for k,v in r.items()}
                out.append(n)
            return out
        except FileNotFoundError:
            continue
    return []
for _r in _loadside_csv():
...
function drawTruckFlow(d,mode){
  ...
  const cxOf=k=>bx+nodeW+(kmC[k]||0)*PXPK;          // full-haul distance to the right of the shovel
  const axOf=k=>bx-(kmA[k]||0)*PXPK-nodeW;          // empty-haul distance to the left of the shovel
  ...
  const H_svg=nodeBot+44;   // extra room for the km ruler + labels below the nodes
  ...
  dumps.forEach(k=>{byDump[k].sort((a,b)=>(a.km||0)-(b.km||0));let yy=posC[k].y;byDump[k].forEach(f=>{f._rh=f.tons*sc;f._ry=yy;f._rx=R2+(f.km||0)*PXPK;yy+=f._rh;});});
  shovels.forEach(k=>{byShov[k].sort((a,b)=>(a.km||0)-(b.km||0));let yy=posB[k].y;byShov[k].forEach(f=>{f._sy=yy;yy+=f.tons*sc;});});
  prevDumps.forEach(k=>{byPrev[k].sort((a,b)=>(a.km||0)-(b.km||0));let yy=posA[k].y;byPrev[k].forEach(f=>{f._ah=f.tons*sc;f._ay=yy;f._ax=bx-(f.km||0)*PXPK;yy+=f._ah;});});
  ...
  let nd=`<text x="${bx-leftAvail/2}" y="${pad-12}" text-anchor="middle" font-size="9.5" font-weight="700" letter-spacing="0.8" fill="#8fa0b8">${DUMPC?'◄ FULL HAUL · SHOVEL':'◄ EMPTY HAUL · PREV DUMP'}</text>`;
  ...
  nd+=`<text x="${bx+nodeW+rightAvail/2}" y="${pad-12}" text-anchor="middle" font-size="9.5" font-weight="700" letter-spacing="0.8" fill="var(--muted)">${DUMPC?'NEXT SHOVEL · EMPTY HAUL ►':'DUMP · FULL HAUL ►'}</text>`;
...
function drawCycleMap(d){
  ...
  const curve=(i,j,tons,c,op,dash,wMul,sign)=>{
    ...
    const w=Math.max(0.6,Math.sqrt(tons/maxTon)*wMul),ang=Math.atan2(by-cy,bx-cx);
  ...
  let eFull='',eEmpty='';
  pf.forEach(f=>{if(idx[f.from]==null||idx[f.to]==null)return;
    eEmpty+=`<g><title>${shortId(f.from)} → ${shortId(f.to)} (empty return): ${f.tons.toLocaleString()}t · ${f.km||0} km</title>${curve(idx[f.from],idx[f.to],f.tons,'#9aa6b5',0.5,'3 3',6,-1)}</g>`;});
  ff.forEach(f=>{if(idx[f.from]==null||idx[f.to]==null)return;const c=f.mat==='Waste'?CWASTE:CORE;
  ...
  return `<svg viewBox="0 0 ${W} ${Hh}" width="100%">${eEmpty}${eFull}${nod}${bar}</svg>`;
}
...
function renderMatPlace(){   // Material Placement Sankey (its own tab) — SVG, renders even without Chart.js
  ...
  document.getElementById('hcmapsrc').textContent=useCo?`— positions from exported x/y (${have}/${ids.size} located)`:'— layout derived from haul distances · add LocationCoordinates.csv (Location,X,Y) for true positions';
  document.getElementById('hc4').innerHTML=drawCycleMap(cyc);
  document.getElementById('hcleg4').innerHTML=`<span class="badge"><b style="color:#2f6f9f">●</b> shovel</span><span class="badge"><b style="color:${CORE}">●</b> ore dump</span><span class="badge"><b style="color:${CWASTE}">●</b> waste dump</span><span class="badge">circle size ∝ tonnes</span><span class="badge"><b style="color:${CORE}">—</b> loaded haul (shovel → dump)</span><span class="badge"><b style="color:#9aa6b5">- -</b> empty return (dump → next shovel)</span><span class="badge">arrowheads show cycle direction · spacing to scale in km</span>`;
}
...
const TABS=[['overview','Shift Overview',0],['matplace','Material Placement',0],['balance','Truck / Shovel Balance',0],['shovel2','Shovel Waterfall',0],['loading','Loading Drill-Down',1],['shovprod','Shovel Productivity',1],['delaysS','Delays & Standby',1],['trucks','Truck Waterfall',0],['haulage','Haulage Drill-Down',1],['truckprod','Truck Productivity',1],['delays','Delays & Standby',1],['hourlyperf','Hourly Production',0],['lube','Fuel and Lube',0],['shiftstats','Shift Stats',0],['trends','Cross-Shift Trends',0],['appendix','Appendix',0],['sandbox','Sandbox',0]];
let tab='overview';
